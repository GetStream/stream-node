 ...
 * that is what `setTimeout` and `ApiConfig.timeout` speak.
 *
 ...
 *   nanosecond timestamp is ~1.79e18, so a bare wire number read as milliseconds lands on an
 *   invalid instance: `new Date(ns).toISOString()` throws `RangeError`, and a date library handed
 *   the same number formats it as the literal string `'Invalid Date'`. Neither is a type error.
...
/**
 * The local clock as a wire-comparable timestamp, for building API-shaped objects.
 *
 * Millisecond resolution, so two calls within the same millisecond produce equal timestamps.
 * Callers that order by timestamp must tie-break on something else.
 */
 ...
 * something a date library or a caller consumes. Use {@link nsToDate} when the value is known to
